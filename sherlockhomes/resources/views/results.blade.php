...
{{-- dd($results->links()) --}}
    	...
    	<div class="container">
        <div>
          <p><b>Resultados:</b> {{$results->total();}}</p>
        </div>
        <div class="row">
			@if($results->total() == 0)
				<img src="\images\results_not_found.png" class="img-fluid rounded mx-auto d-block" alt="Responsive image">
			@endif
        @foreach($results as $propertie)
        			<?php
			...
			<div class="col-md-4">
				...
				{{-- ! a linha abaixo tem que ser analisada --}}
        			...
        			<div class="text" onclick="location.href='/prop/{{ $propertie->id }}';" style="cursor:pointer;">
						...
						@if($propertie->typeprice_id == 1)
							<?php
								$prec_med = $prec_med_comp;
							?>
						@else
							<?php
								$prec_med = $prec_med_alug;
							?>
						@endif
						@if($propertie->price < ($prec_med * 0.9))
        				<p class="price"><span class="orig-price" style="color:green;">{{number_format($propertie->price, 0,',','.').'€' }}
											@if($propertie->typepropertie_id == 2)
												<small>/mês</small>
											@endif
										 </span>
						@elseif($propertie->price > ($prec_med * 1.1))
        				<p class="price"><span class="orig-price" style="color:red;">{{number_format($propertie->price, 0,',','.').'€' }}
											@if($propertie->typepropertie_id == 2)
												<small>/mês</small>
											@endif
										 </span>
...
{{-- 		
			<div class="col-md-4">
        		<div class="property-wrap ftco-animate">
        			<div href="" class="img" style="background-image: url(images/work-1.jpg); ">
					@if(Auth::user() != null)
						<div><button>Adicionar Favoritos</button></div>
					@endif
					</div>
        			<div class="text">
